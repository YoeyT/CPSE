...
std::string rtttl_convert( note_player & lsp, const char *s ){
   int def_duration = 4, def_octave = 6, beat = 100, value;
   int duration, octave, frequency;
   int state = 0;
   char def;
   bool dot;
   std::string rtttl_string = "#include \"melody.hpp\"\n\nclass converted_melody : public melody {\npublic:\n    void play( note_player & p ){\n";
   for( const char * p = s; state >= 0; p++ ){
      const char c = *p;
      ...
      switch(state ){
         ...
         case 10:   
            if( ( c == ',' ) || ( c == '\0') ){
               ...
               rtttl_string += lsp.convert( note{ frequency / 1000, note_duration } ); 
